 ...
 *
 * <p>Each connection can carry a varying number streams, depending on the underlying protocol
 * being
 * used. HTTP/1.x connections can carry either zero or one streams. HTTP/2 connections can carry
 * any
 * number of streams, dynamically configured with {@code SETTINGS_MAX_CONCURRENT_STREAMS}. A
 * connection currently carrying zero streams is an idle stream. We keep it alive because reusing
 * an
...
public interface Connection {
     ...
     * javax.net.ssl.SSLSocket SSL socket} if this connection is HTTPS. If this is an HTTP/2 or
     * SPDY
    ...
    /**
     * Returns the TLS handshake used to establish this connection, or null if the connection is
     * not
     * HTTPS.
     */
    Handshake handshake();
     ...
     * protocol
     * has been negotiated. This method returns {@link Protocol#HTTP_1_1} even if the remote peer
     * is
