...
public interface RepositoryCompilerStates {
    ...
    interface RFrequency<TVal, TStart> extends REventSource<TVal, TStart> {
         ...
         * before starting another flow to respond to updates from the event sources. Flows will not
         * be
    ...
    interface RSyncFlow<TVal, TPre, TSelf extends RSyncFlow<TVal, TPre, TSelf>> {
         ...
         * given
         * supplier. If the attempt fails, terminate the data processing flow by sending the failure
         * to
         ...
         * not
         * guarantee value immutability or concurrent access for this receiver. For this reason, for
         * a
         * UI-calling receiver invoked from a background thread, implementation should extract any
         * necessary data from the input value, and post the immutable form of it to the main thread
         * for
         * the UI calls, so the UI modifications are main-thread-safe while the data processing flow
         * can
    ...
    interface RConfig<TVal> {
         ...
         * preventative measure to handle a potentially different value which is unknown at the time
         * of
         ...
         * {@code checker} will be consulted, but because the reset happens only when the repository
         * is
