 ...
 * </ul>
 *
 * Sometimes a request will result in a conditional cache hit. If the cache contains a stale copy
 * of
 ...
 * href="http://tools.ietf.org/html/rfc7234">HTTP/1.1 (RFC 7234)</a> cache headers, it doesn't
 * cache
 * partial responses.
 *
 ...
 *     }
 * }</pre>
 * This technique works even better in situations where a stale response is better than no
 * response.
...
public final class Cache implements Closeable, Flushable {
     ...
     *
     * <p>The initialization time may vary depending on the journal file size and the current
     * actual
    ...
    private static final class Entry {
         ...
         *
         * <p>HTTPS responses also contain SSL session information. This begins with a blank line,
         * and
         * then a line containing the cipher suite. Next is the length of the peer certificate
         * chain.
         * These certificates are base64-encoded and appear each on their own line. The next line
         * contains the length of the local certificate chain. These certificates are also
         * base64-encoded and appear each on their own line. A length of -1 is used to encode a
         * null
        ...
        public Entry(Source in) throws IOException {
            try {
                ...
                String receivedResponseMillisString = responseHeadersBuilder.get(
                        OkHeaders.RECEIVED_MILLIS);
                responseHeadersBuilder.removeAll(OkHeaders.SENT_MILLIS);
                responseHeadersBuilder.removeAll(OkHeaders.RECEIVED_MILLIS);
                sentRequestMillis = sendRequestMillisString != null ? Long.parseLong(
                        sendRequestMillisString) : 0L;
                receivedResponseMillis = receivedResponseMillisString != null ? Long.parseLong(
                        receivedResponseMillisString) : 0L;
                ...
                if (isHttps()) {
                    ...
                    TlsVersion tlsVersion = !source.exhausted() ? TlsVersion.forJavaName(
                            source.readUtf8LineStrict()) : null;
