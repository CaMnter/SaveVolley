...
/**
 * Handles a single HTTP request/response pair. Each HTTP engine follows this lifecycle: <ol>
 * <li>It
...
public final class HttpEngine {
     ...
     * streamed. Buffering has two advantages: we don't need the content-length in advance and we
     * can
    ...
    /**
     * True if this client added an "Accept-Encoding: gzip" header field and is therefore
     * responsible
     ...
     * Conditional caching means this may be non-null even when the network request is non-null.
     * Never
     * modified by OkHttp.
     */
    private Response cacheResponse;
    /**
     * The user-visible response. This is derived from either the network response, cache response,
     * or
     ...
     * model
     * where control flow is returned to the calling application to write the request body before
     * the
    ...
    public HttpEngine(OkHttpClient client, Request request, boolean bufferRequestBody, boolean callerWritesRequestBody, boolean forWebSocket, StreamAllocation streamAllocation, RetryableSink requestBodyOut, Response priorResponse) {
        ...
        this.streamAllocation = streamAllocation != null
                                ? streamAllocation
                                : new StreamAllocation(client.connectionPool(),
    ...
    /**
     * Figures out what the response source will be, and opens a socket to that source if
     * necessary.
     ...
     * @throws RouteException if the was a problem during connection via a specific route.
     * Sometimes
    ...
    private boolean writeRequestHeadersEagerly() {
        return callerWritesRequestBody && permitsRequestBody(networkRequest) &&
                requestBodyOut == null;
    }
    ...
    private HttpStream connect() throws RouteException, RequestException, IOException {
        ...
        return streamAllocation.newStream(client.connectTimeoutMillis(), client.readTimeoutMillis(),
                client.writeTimeoutMillis(), client.retryOnConnectionFailure(),
                doExtensiveHealthChecks);
    }


    /**
     * Called immediately before the transport transmits HTTP request headers. This is used to
     * observe
     ...
     * HTTP
     * engine that should be used for the retry if {@code e} is recoverable, or null if the failure
     * is
     ...
     * interrupt an in-flight request from any thread. It's the caller's responsibility to close
     * the
     * request body and response body streams; otherwise resources may be leaked.
     *
     * <p>This method is safe to be called concurrently, but provides limited guarantees. If a
     * transport layer connection has been established (such as a HTTP/2 stream) that is
     * terminated.
    ...
    /**
     * Release any resources held by this engine. Returns the stream allocation held by this
     * engine,
     ...
     *
     * <p>In addition to decompression, this will also strip the corresponding headers. We strip
     * the
     ...
     *
     * <p>This method should only be used for non-empty response bodies. Response codes like "304
     * Not
     * Modified" can include "Content-Encoding: gzip" without a response body and we will crash if
     * we
    ...
    /**
     * Returns a new source that writes bytes to {@code cacheRequest} as they are read by the
     * source
    ...
    /**
     * Figures out the HTTP request to make in response to receiving this engine's response. This
     * will
    ...
    public Request followUpRequest() throws IOException {
        ...
        switch (responseCode) {
            ...
            case HTTP_CLIENT_TIMEOUT:
                ...
                boolean retryableBody = requestBodyOut == null ||
                        requestBodyOut instanceof RetryableSink;
            ...
            if (index > 0) {
                ...
                if (calls > 1) {
                    throw new IllegalStateException(
                            "network interceptor " + caller + " must call proceed() exactly once");
                }
            }
            ...
            if (index < client.networkInterceptors().size()) {
                ...
                if (interceptedResponse == null) {
                    throw new NullPointerException(
                            "network interceptor " + interceptor + " returned null");
                }
